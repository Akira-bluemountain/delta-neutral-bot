...
- 動作環境: **Docker Compose** (🍎 Mac / 🪟 Windows / 🐧 Linux すべて対応)

## できること

- 35 銘柄を常時スクリーニングし、FR 差が一定以上に開いたペアを自動エントリー
...
> 💡 **🪟 Windows ユーザーへ**: ターミナルは「PowerShell」を使ってください。
> 詳しい手順は [SETUP.md](./SETUP.md) を参照(OS 別に分けて解説しています)。
...
3. **設定ファイル作成**
   ...
   `.env` をエディタで開き、API キー 6 項目を入力(詳細は [SETUP.md](./SETUP.md))。
...
- [SETUP.md](./SETUP.md) — 口座開設〜API キー取得〜起動までの完全手順(🍎 Mac / 🪟 Windows / 🐧 Linux 対応)
- [TROUBLESHOOTING.md](./TROUBLESHOOTING.md) — うまく動かないときの対処集(OS 別の問題も解説)
