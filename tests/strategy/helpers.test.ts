...
// Task A2 で確定した新値。テストではこの値を固定して境界を検証する。
const FR_OPEN = 0.00025;
const FR_CLOSE = 0.00010;
const MIN_HOLD_MINUTES = 180;

describe("meetsFrThreshold (frOpen=0.00025)", () => {
  ...
  it("旧値 frOpen=0.00005 では 0.00005 で通過、新値 0.00025 では同じ入力は拒否", () => {
    const oldFrOpen = 0.00005;
    const rateAtOldThreshold = 0.00005;
    expect(meetsFrThreshold(rateAtOldThreshold, 0, oldFrOpen)).toBe(true);
    expect(meetsFrThreshold(rateAtOldThreshold, 0, FR_OPEN)).toBe(false);
  });
});
...
describe("isFrSpreadCollapsed (frClose=0.00010)", () => {
  ...
  it("hlRate と extRate が符号反対でも絶対差で評価される", () => {
    // spread = |0.00008 − (−0.00003)| = 0.00011 > frClose=0.00010 → 消滅しない
    expect(isFrSpreadCollapsed(0.00008, -0.00003, FR_CLOSE)).toBe(false);
    // spread = |0.00005 − (−0.00002)| = 0.00007 < frClose=0.00010 → 消滅
    expect(isFrSpreadCollapsed(0.00005, -0.00002, FR_CLOSE)).toBe(true);
  });
...
describe("shouldClosePair early-return 構造の論理検証（Task A2 §A2-2-1-3）", () => {
  ...
  it("179 分時点でスプレッド消滅条件（spread < frClose）が揃っていても、isWithinMinHold=true なので判定は走らない", () => {
    // シナリオ: 両ベニュー FR がほぼ一致（spread=0.00001 < frClose=0.00010）。
